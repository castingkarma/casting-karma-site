...
// Generates the discovery "reading" with OpenAI (gpt-4o-mini) when OPENAI_API_KEY is set on
// the Pages project. Until a key exists it returns {ok:false} and the site falls back to its
// built-in composer, so the tool always works. Cheap by design (gpt-4o-mini, ~220 output tokens
// => fractions of a cent per reading). Runs on the existing LLC OpenAI org (per-product key +
// spend cap), the same account that powers HullDocs - no new account, no new bill.
//
// Turn ON: OpenAI org "Casting Karma LLC" - create a per-product API key (project e.g. "CK-Website")
// with a spend cap - then Cloudflare dashboard - Workers and Pages - casting-karma-site - Settings -
// Variables and Secrets - add OPENAI_API_KEY (secret) - redeploy.
